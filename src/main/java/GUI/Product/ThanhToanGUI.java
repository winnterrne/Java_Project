...
//        });
    ...
    // =========================================================
// Bạn cần truyền thêm danh sách giỏ hàng từ giao diện Bán Hàng vào hàm này
    public void capNhatThongTin(HoaDon_DTO hoaDon, ArrayList<ChiTietHoaDon_DTO> gioHang) {
        this.hoaDonHienTai = hoaDon;

        // 1. TÌM KHÁCH HÀNG THÔNG QUA MÃ KHÁCH HÀNG
        KhachHang_DTO kh = null;
        if (hoaDon.getMaKH() != null && !hoaDon.getMaKH().isEmpty()) {
            // Giả sử bạn có class KhachHang_BUS chứa hàm getKhachHangByMa
            KhachHang_BUS khBus = new KhachHang_BUS();
            kh = khBus.layKHTheoMaKH(hoaDon.getMaKH());
        }
        ...
        // 3. TÍNH TỔNG TIỀN TỪ DANH SÁCH GIỎ HÀNG (Sửa lỗi dùng nhầm BUS thay vì DTO)
        double tongTien = 0;
        if (gioHang != null) {
            for (ChiTietHoaDon_DTO cthd : gioHang) { // Dùng DTO thay vì BUS
                tongTien += cthd.getThanhTien();
            }
        }

        // 4. FORMAT VÀ HIỂN THỊ TIỀN
