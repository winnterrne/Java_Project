...
public class SuaPhieuNhapGUI extends JFrame {
    JPanel lPanel, lbPanel, rPanel, rbPanel, pSearch, pTongTien;
    ...
    JComboBox cbNhaCC;
    JButton btnSuaSL, btnXoaSP, btnLuu;
    ...
    ChiTietPhieuNhap_DAO ctDAO = new  ChiTietPhieuNhap_DAO();
    public SuaPhieuNhapGUI(String maPN) {
        this.maPN = maPN;
        pnDTO = pnDAO.getPhieuNhapbyMaPN(maPN);
        initGUI();
        loadTableSanPham();
        loadNCC();
        loadChiTiet();
    ...
    public void initGUI() {
        ...
        lTinhTongTien = new JLabel(tongTienFormat(pnDTO.getTongTien()));
        lTinhTongTien.setFont(new Font("Times New Roman", Font.BOLD, 15));
        lTinhTongTien.setForeground(Color.RED);
        pTongTien.add(lTongTien, rgbc);
        pTongTien.add(lTinhTongTien, rgbc);
        rbPanel.add(pTongTien, rbgbc);
    ...
    public void loadNCC() {
        ...
        cbNhaCC.setSelectedItem(nccBUS.getTenNCCByMaNCC(pnDTO.getMaNCC()));
    }

    public void loadChiTiet() {
        ArrayList<ChiTietPhieuNhap_DTO> ctDTO = ctBUS.getChiTietPhieuNhapByMaPN(maPN);

        modelChiTiet.setRowCount(0);
        int stt = 1;
        NumberFormat nf = NumberFormat.getInstance(new Locale("vi", "VN"));

        for(ChiTietPhieuNhap_DTO ct : ctDTO) {
            String tenSP = ctDAO.getTenSPByMaSP(ct.getMaSP());
            String giaNhapFormatted = nf.format(ct.getGiaNhap());
    ...
    public String tongTienFormat(double tongTien) {
        NumberFormat nf =  NumberFormat.getInstance(new  Locale("vi", "VN"));
        String numberFormatted =  nf.format(tongTien);
        return numberFormatted;
    }
